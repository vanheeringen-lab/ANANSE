...
def gff_enrichment(sample, background, numsample, numbackground, outfile):
    ...
    (e, p, ns, nb) = calc_motif_enrichment(
        data_sample, data_bg, "Benjamini-Hochberg", numsample, numbackground
    )
...
def motif_localization(fastafile, motif, width, outfile, cutoff=0.9):
    ...
    matches = motif.pfm_scan(Fasta(fastafile), cutoff=cutoff, nreport=NR_HIST_MATCHES)
    if len(matches) > 0:
        ar = []
